...
**Cache key naming**: `<source>-<entity>[-scope...]` where `source` ∈ {`fantrax`, `mlb`, `fangraphs`, `savant`, `hkb`, `prospect`, `mlb-handedness`}, `entity` is the data shape (e.g. `roster-stats`, `pitcher-gs`, `matchups`, `schedule`, `bat`, `pit`), and `scope` is whatever disambiguators apply (teamID, period, date). Examples: `fantrax-roster-stats-<teamID>-<period>` (per-period team snapshot, 30d TTL), `fantrax-pitcher-gs-<teamID>-<period>` (per-period pitcher GS snapshot, 30d TTL), `mlb-schedule-<YYYY-MM-DD>` (past-date MLB schedule, 30d TTL — today/future always fetched fresh), `fangraphs-bat-rotopitcher` (Steamer batters JSON, 12h TTL). Old `backtest-snapshot-mlb-*` files (pre–roster-stats rename) and `backtest-snapshot-*` files (StatsType=2 era) are orphaned and safe to delete.
