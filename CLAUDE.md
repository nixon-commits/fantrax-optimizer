...
**`internal/config`** — loads env vars via `godotenv` and returns a `Config` struct. The `PLATFORM` env var (default `"fantrax"`, accepts `"espn"`) selects the fantasy provider. Validation is platform-aware: when `PLATFORM=fantrax` the four `FANTRAX_*` vars are required; when `PLATFORM=espn` the four `ESPN_*` vars (`ESPN_LEAGUE_ID`, `ESPN_TEAM_ID`, `ESPN_SWID`, `ESPN_S2`) are required and Fantrax vars are not. `cmd/root.go:initApp` rejects `PLATFORM=espn` with a clear error since only the `waivers` command currently has an ESPN adapter.
...
**`internal/waivers`** — Statcast-driven waiver wire audit. `Run(p, today, opts)` takes a `waivers.Platform` interface (defined in `platform.go`) so both Fantrax and ESPN can drive the same report. The interface is intentionally tiny (5 read-only methods: `GetFreeAgents`, `GetHitterScoringWeights`, `GetPitcherScoringWeights`, `GetHitterRoster`, `GetPitcherRoster`) and uses the package's own neutral types (`FreeAgent`, `RosteredPlayer`) — no shared types with `internal/fantrax` or `internal/espn`. Adapters live in `cmd/waivers_adapters.go` (`fantraxWaiversAdapter`, `espnWaiversAdapter`) where they convert each platform's native shape into the neutral types. The ESPN adapter caches the league `Settings` via `sync.Once` so the parallel `errgroup` in `Run` doesn't double-fetch. `Run` then resolves names → MLBAM IDs via `playername.ResolveMLBAMIDs`, and joins each FA against (a) FanGraphs projections from `projections.LoadBattingProjections`/`LoadPitcherProjections` for the per-game point projection (default system: `depthcharts`, matching the `optimize` command; override via `--projections` or `Options.ProjectionSystem`), and (b) a `SavantBundle` of five Baseball Savant CSVs for signal tagging. Two signals fire per player: **BUY-LOW** when xStats outpace surface stats (hitter: `xwOBA - wOBA ≥ 0.030` AND barrel ≥ 9 AND hard-hit ≥ 42; pitcher: `era - xera ≥ 1.00` AND `xwoba ≤ 0.310`) and **HOT** when recent production is backed by quality (hitter: 14d `wOBA ≥ .380` AND `xwOBA ≥ .360` AND season barrel ≥ 8; pitcher: 30d `era ≤ 3.20` AND `xera ≤ 3.50`). When both fire, the row is tagged `BOTH`. Sample guards: hitter season ≥80 PA, hitter 14d ≥20 PA, pitcher season ≥100 TBF, pitcher 30d ≥50 TBF. Defaults live in `DefaultThresholds()`; tests override. The design separates concerns deliberately — Statcast picks WHO surfaces, Steamer scores HOW MUCH — so the league's own `ScoringWeights` (via `ExpectedPtsFromProj`) drive the ranking and FAs without a Steamer projection are skipped (accepted v1 limitation). Output: stdout (always), GHA markdown summary (when `GITHUB_STEP_SUMMARY` set), Pushover (when `!DryRun` and creds set; `formatPushover` truncates to fit the 1024-char limit). CSV fetchers in `savant.go` have URLs as `var` for test override; column lookup is by lowercased name with aliases so Savant header drift doesn't break parsing. CSVs are cached at 12h TTL via `cache.FileCache[T]` keyed by year + window-end-date.
...
**`internal/espn`** — thin HTTP client for ESPN's unofficial Fantasy Baseball v3 API (`https://lm-api-reads.fantasy.espn.com/apis/v3/games/flb/...`). Read-only support for the `waivers` and `recap` commands in daily-lock H2H Points leagues. Auth is two cookies (`SWID`, `espn_s2`) the user grabs from Chrome DevTools — vastly simpler than Fantrax's chromedp dance. Exposes `NewClient(leagueID, teamID, year, swid, espn_s2)` plus three reads: `GetSettings()` (parses `mSettings` view, splits scoring categories into hitter/pitcher weight maps via the `HitterStatID`/`PitcherStatID` translation tables), `GetTeamRoster(teamID)` (uses `mRoster`+`mTeam`), and `GetFreeAgents(limit)` (uses `players` endpoint with an `X-Fantasy-Filter` JSON header that filters status to `FREEAGENT`/`WAIVERS` and sorts by percent-owned).

**Free agent endpoint gotcha** — the FA query MUST hit the per-league URL `/leagues/{leagueId}?view=kona_player_info`, NOT the global `/seasons/{year}/players` endpoint. The global endpoint returns all 23k+ MLB+MiLB players regardless of league ownership (no league context to consult), so `filterStatus: FREEAGENT/WAIVERS` gets effectively ignored and rostered players (other teams' aces) leak in as "FAs". The X-Fantasy-Filter must include `filterRanksForScoringPeriodIds` (any valid period works — 1 = opening day is the stable default) or ESPN rejects the request with `Filter: Value missing` (HTTP 400). The league response wraps each player under a `players[].player` envelope; `playerPoolResponse` reflects that.

**Stat ID translation (`constants.go`)** is the bridge that makes ESPN data consumable by the project's existing scoring engine. ESPN uses different statIds for hitter vs pitcher versions of the "same" stat (statId 10 is hitter walks, 39 is pitcher walks — both must surface as `BB` in their respective weight map), so the table is split into `HitterStatID` and `PitcherStatID` (mutually exclusive). Three special-cases worth knowing:
- statId **34** is `OUTS` (1 point per out); the project's `PitcherExpectedPtsFromProj` only knows `IP`, so `league.go` multiplies the points by 3 and stores them under `IP` instead. A 1-pt-per-out league becomes a 3-pt-per-IP league as far as the scoring engine is concerned.
- statId **51** is `BLK` (balk) per ESPN, but the project's pitcher projection key is `BK` — the table renames.
- statId **52** is `PK` (pickoff) per ESPN, project key is `PKO` — also renamed.

Stat IDs the project's scoring engine doesn't recognize (rate stats like `K/9`, totals like `TBF`, no-hit/perfect-game bonuses) are intentionally absent from the tables — silently dropped rather than producing phantom keys nothing consumes. Position normalization (`roster.go`) collapses ESPN's `LF`/`CF`/`RF`/`OF` slots to a single canonical `OF` so downstream eligibility checks see the project's standard short names. The `/players` endpoint returns a flat array of player records (NOT wrapped in `playerPoolEntry` envelopes the way the league/roster endpoint does) — the `playerPoolResponse` type reflects that. The package owns its own `Player`/`Settings` types and never imports `internal/fantrax` — neutralization happens in the cmd-layer adapter (`cmd/waivers_adapters.go:espnWaiversAdapter`). `client.go` exposes `SetBaseURL` for httptest fixtures.

**Defense in `waivers.Run`** — ESPN's `filterStatus` filter excludes owned players by player ID, but MiLB players or duplicate ESPN player records sharing a name with someone the user owns can still leak through (Eury Perez SF prospect ≠ Eury Perez MIA ace; Steven Kwan has 3 different ESPN player IDs). `excludeRosteredNames` runs after the FA fetch and drops any FA whose normalized name matches a rostered player. No-op for Fantrax (which dedupes by ID server-side).

**Recap support** — `internal/espn/season.go`, `teams.go`, `matchups.go`, `boxscore.go` add the methods recap needs. Two endpoint gotchas:
- `mScoreboard` returns the schedule but leaves `matchupPeriodId=0` on every entry — useless for grouping. `mMatchupScore` populates it correctly, so `GetMatchups` uses that view.
- `settings.scheduleSettings.matchupPeriods` lies — it claims each matchup period contains a single scoring period even when the matchup actually spans 7-12 days. The truth lives in each matchup's `pointsByScoringPeriod` keys, so `GetSeasonInfo` reconstructs the matchup→scoring-period map from `mMatchupScore` rather than trusting settings.

**Slot ID translation for recap** lives in `cmd/recap_adapters_espn.go` (`espnSlotToFantrax`/`espnSlotToFantraxPosName`). ESPN slot IDs are remapped to the Fantrax position ID strings the optimizer/backtest expect (e.g. ESPN OF=5 → Fantrax "012", ESPN UTIL=12 → Fantrax "014" with `PosName="UT"` to trigger the special-case eligibility check). DH (ESPN slot 11) collapses to UT since Fantrax has no separate DH slot.

**Deferred:** writes (lineup apply via POST endpoints — needs reverse engineering from browser network traffic) and weekly-lock H2H cadence support. Pitcher-start filtering: ESPN GS check is currently bypassed (any nonzero-FPts SP/P-slot day counts as a "start"), which slightly inflates Best/Worst single-start awards in leagues where pitchers swing between SP and relief. Also deferred: a project-wide `Platform` interface in a neutral `internal/league` package. The current design has per-consumer `Platform` interfaces (`waivers.Platform`, `recap.Platform`), each with its own adapters in `cmd/`. `optimize`/`backtest`/`prospects`/`transactions`/`gscheck` are still Fantrax-only and `cmd/root.go:initApp` rejects `PLATFORM=espn`. Add ESPN to those consumers in a follow-up session, not as part of unrelated work.
...
`.github/workflows/waivers.yml` runs daily at 1pm UTC (9am ET) and on `workflow_dispatch` (with `dry_run`, `top`, and `platform` inputs — `platform` is `fantrax` or `espn`, default `fantrax`). Two jobs: `audit-waivers-fantrax` runs on every cron and on dispatch when `platform=fantrax`; `audit-waivers-espn` runs only on dispatch when `platform=espn` and uses the `ESPN_LEAGUE_ID` / `ESPN_TEAM_ID` / `ESPN_SWID` / `ESPN_S2` secrets (these must be added before the ESPN job will succeed). Cron stays Fantrax-only until the ESPN MVP is proven in production. Sends Pushover when not in dry-run. Uses `actions/cache@v4` with key prefix `waivers-` (Fantrax) or `waivers-espn-` (ESPN), both falling back to `projections-` so the Steamer JSON and Savant CSVs survive across runs.
