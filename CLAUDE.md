...
**Deferred:** writes (lineup apply via POST endpoints — needs reverse engineering from browser network traffic) and weekly-lock H2H cadence support. Also deferred: a project-wide `Platform` interface in a neutral `internal/league` package — the current design keeps the abstraction scoped to `waivers` only, so `optimize`/`recap`/`backtest`/`prospects`/`transactions`/`gscheck` are still Fantrax-only and `cmd/root.go:initApp` rejects `PLATFORM=espn`. Add ESPN to those consumers in a follow-up session, not as part of unrelated work.
